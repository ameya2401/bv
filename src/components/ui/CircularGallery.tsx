...
async function loadFontFromStylesheet(url: string) {
  ...
  for (const block of faceBlocks) {
    ...
    if (weightMatch) (descriptors as any).weight = weightMatch[1].trim();
    if (styleMatch) (descriptors as any).style = styleMatch[1].trim();
    if (rangeMatch) (descriptors as any).unicodeRange = rangeMatch[1].trim();
    fontFaces.push(new FontFace(family, `url(${urlMatch[1]})`, descriptors as any));
  }
